...
function ParseGameEvents(gameEvents)
{
  if (!InTransition)
  {
    for (let i = 0; i < gameEvents.length; i++)
    {
      ...
      if (gameEvents[i].substring(0, 4) == "load")
      {     
        LoadScene(gameEvents[i].substring(5, gameEvents[i].length));
      }
    }
...
//#region not used
function CheckButtonsGUI()
{
  for (let i = 0; i < ActiveButtons.length; i++)
  {
    result = ActiveButtons[i].IsPressed();
    if (result != "")
    {
      continue;
    }

    if (result.substring(0, 4) == "load")
    {
        LoadScene(result.substring(5, result.length-1));
    }
  }
}
//#endregion
